...
- Delete all tiddlers contained `Mono` in the file name or path.
- Overwrite `myStyleSheet` to `myStyleSheet.css`, and copy all content from a newer file to it.
- Check if there is a tiddler named  `Embed`  or not. If not, add a new one.
...
.tc-topbar {
   background-color: <<colour topbar-color>>;
}
...
external-link-foreground-visited: #5881d3
external-link-foreground: #22a087
foreground: #333333
